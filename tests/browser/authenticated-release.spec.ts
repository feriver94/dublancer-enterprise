...
test("manual profile UI and media lifecycle", async ({ page }, testInfo) => {
  ...
  await page.getByRole("button", { name: "Freelancer profile" }).click();
  const media = page.getByRole("region", { name: "Profile photo" });
  const input = media.locator('input[type="file"]');
  await input.setInputFiles({ name: "avatar.png", mimeType: "image/png", buffer: png });
  await media.getByRole("button", { name: "Confirm upload" }).click();
  await expect(page.locator('.profile-settings > p[role="status"]')).toHaveText("Profile image uploaded successfully.", { timeout: 30_000 });
  await expect(media.getByRole("button", { name: "Change photo" })).toBeVisible();
  const firstPreview = media.locator(".profile-media__preview");
  await expect(firstPreview).toHaveAttribute("style", /\/api\/profile\/media\//);
  const firstStyle = await firstPreview.getAttribute("style");
  await expect(page.getByRole("button", { name: "User profile" }).locator('[style*="profile/media"]')).toBeVisible();
  await page.locator("form.profile-form").getByRole("button", { name: "Save changes" }).click();
  await expect(page.locator('.profile-settings > p[role="status"]')).toHaveText("Changes saved");

  await page.reload({ waitUntil: "domcontentloaded" });
  await page.getByRole("button", { name: "Freelancer profile" }).click();
  const persisted = page.getByRole("region", { name: "Profile photo" });
  await expect(persisted.getByRole("button", { name: "Change photo" })).toBeVisible();
  await expect(persisted.locator(".profile-media__preview")).toHaveAttribute("style", firstStyle ?? "");
  await expect(page.getByRole("button", { name: "User profile" }).locator('[style*="profile/media"]')).toBeVisible();
  await page.getByRole("button", { name: "User profile" }).click();
  await expect(page.getByRole("dialog", { name: "User profile" }).locator('[style*="profile/media"]')).toBeVisible();
  await page.keyboard.press("Escape");
  ...
  await persisted.getByRole("button", { name: "Confirm upload" }).click();
  await expect(page.locator('.profile-settings > p[role="status"]')).toHaveText("Profile image uploaded successfully.", { timeout: 30_000 });
  await expect(persisted.locator(".profile-media__preview")).not.toHaveAttribute("style", firstStyle ?? "");
  await page.reload({ waitUntil: "domcontentloaded" });
  await page.getByRole("button", { name: "Freelancer profile" }).click();
  const replaced = page.getByRole("region", { name: "Profile photo" });
  await expect(replaced.getByRole("button", { name: "Remove photo" })).toBeVisible();
  await expect(replaced.locator(".profile-media__preview")).not.toHaveAttribute("style", firstStyle ?? "");
  await replaced.getByRole("button", { name: "Remove photo" }).click();
  await expect(page.locator('.profile-settings > p[role="status"]')).toHaveText("Profile image removed.", { timeout: 30_000 });
  await expect(replaced.getByRole("button", { name: "Upload photo" })).toBeVisible();
  await page.reload({ waitUntil: "domcontentloaded" });
  await page.getByRole("button", { name: "Freelancer profile" }).click();
  await expect(page.getByRole("region", { name: "Profile photo" }).getByRole("button", { name: "Upload photo" })).toBeVisible();
  await expect(page.getByRole("button", { name: "User profile" })).toContainText("BM");
  await page.getByRole("button", { name: "User profile" }).focus();
  await page.keyboard.press("Enter");
  const accountPanel = page.getByRole("dialog", { name: "User profile" });
