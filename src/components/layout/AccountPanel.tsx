  ...
  return createPortal(<div className="fixed inset-0 z-[70] bg-slate-950/30" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}>
    <div ref={panel} role="dialog" aria-modal="true" aria-label={labels.profile} onKeyDown={trap} className="absolute inset-x-0 bottom-0 max-h-[calc(100vh-4rem)] overflow-y-auto rounded-t-3xl border border-slate-200 bg-white p-4 text-slate-800 shadow-2xl sm:inset-x-auto sm:bottom-auto sm:end-4 sm:top-20 sm:w-[390px] sm:max-h-[calc(100vh-6rem)] sm:rounded-3xl">
      ...
      {personas.length ? <section className="mt-4 border-t border-slate-100 pt-4" aria-label={labels.switchPersona}>
        ...
        <div className="grid gap-1">{personas.map((persona) => <button key={persona.id} type="button" disabled={Boolean(busyPersonaId) || persona.id === activePersonaId} onClick={() => onSwitchPersona(persona.id)} aria-current={persona.id === activePersonaId ? "true" : undefined} className={`rounded-xl px-3 py-2.5 text-start hover:bg-slate-50 focus-visible:outline-2 focus-visible:outline-emerald-600 disabled:opacity-70 ${persona.id === activePersonaId ? "bg-emerald-50 text-emerald-800" : ""}`}><strong className="block text-sm">{busyPersonaId === persona.id ? labels.switchingPersona : persona.label}</strong><span className="block truncate text-xs text-slate-500">{persona.type} · {persona.organizationName}</span></button>)}</div>
