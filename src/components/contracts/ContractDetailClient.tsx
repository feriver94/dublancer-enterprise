...
export default function ContractDetailClient({ contractId }: { contractId: string }) {
  ...
  const phaseC = useTranslations("PhaseC");
  ...
  return <main className="py-12">
    ...
    <div className="mt-4 grid gap-6 lg:grid-cols-[minmax(0,1fr)_350px]">
      <section className="grid gap-6">
        <Card variant="elevated"><div className="flex flex-wrap justify-between gap-4"><div><p className="font-bold text-[#009A44]">{row.project?.title ?? row.listing?.title ?? t("agreement")}</p><h1 className="mt-2 text-4xl font-bold text-[#0F4C5C]">{row.title}</h1><p className="mt-2 text-sm font-bold text-[#0F4C5C]">{row.viewerParty === "CLIENT" ? phaseC("actingClient") : phaseC("actingProvider")}</p></div><Badge variant={row.status === "ACTIVE" || row.status === "COMPLETED" ? "success" : "info"}>{label(row.status)}</Badge></div><dl className="mt-6 grid grid-cols-2 gap-4 md:grid-cols-4"><Metric label={t("value")} value={money(row.valueMinor)} /><Metric label={t("tax")} value={`${row.taxRateBasisPoints / 100}%`} /><Metric label={t("platformFee")} value={`${row.platformFeeBasisPoints / 100}%`} /><Metric label={t("milestones")} value={String(row.milestones.length)} /></dl></Card>
        ...
        <Card variant="elevated"><h2 className="mb-4 text-xl font-bold text-[#0F4C5C]">{t("reviews")}</h2>{row.status === "COMPLETED" && !row.reviews.some((review) => review.reviewerParty === row.viewerParty) ? <form className="enterprise-form mb-5" onSubmit={(event) => void formMutation(event, "review:create", `/api/contracts/${contractId}/reviews`, "POST", (data) => ({ overall: Number(data.get("overall")), communication: Number(data.get("communication")), ...(row.viewerParty === "CLIENT" ? { quality: Number(data.get("quality")), delivery: Number(data.get("delivery")), expertise: Number(data.get("expertise")), professionalism: Number(data.get("professionalism")) } : { hiringClarity: Number(data.get("hiringClarity")), paymentReliability: Number(data.get("paymentReliability")), professionalConduct: Number(data.get("professionalConduct")) }), title: data.get("title") || undefined, body: data.get("body") }), t("reviewPublished"))}><div className="grid gap-3 sm:grid-cols-2"><RatingField name="overall" label={phaseC("overall")} /><RatingField name="communication" label={phaseC("communication")} />{row.viewerParty === "CLIENT" ? <><RatingField name="quality" label={phaseC("quality")} /><RatingField name="delivery" label={phaseC("delivery")} /><RatingField name="expertise" label={phaseC("expertise")} /><RatingField name="professionalism" label={phaseC("professionalism")} /></> : <><RatingField name="hiringClarity" label={phaseC("hiringClarity")} /><RatingField name="paymentReliability" label={phaseC("paymentReliability")} /><RatingField name="professionalConduct" label={phaseC("professionalConduct")} /></>}</div><label>{common("title")}<input name="title" /></label><label>{t("reviewBody")}<textarea name="body" minLength={3} required /></label><Button disabled={Boolean(pending)}>{t("writeReview")}</Button></form> : null}{row.reviews.length ? <div className="grid gap-3">{row.reviews.map((review) => <article key={review.id} className="rounded-xl border p-4"><div className="flex justify-between gap-3"><strong>{review.title || review.reviewer.displayName}</strong><span aria-label={`${review.rating} / 5`}>{"★".repeat(review.rating)}{"☆".repeat(5 - review.rating)}</span></div><p className="mt-2 text-sm">{review.body}</p></article>)}</div> : <p className="enterprise-empty">{t("noReviews")}</p>}</Card>
...
function RatingField({ name, label }: { name: string; label: string }) { return <label>{label}<select name={name} defaultValue="5">{[5,4,3,2,1].map((rating) => <option key={rating} value={rating}>{rating} / 5</option>)}</select></label>; }
