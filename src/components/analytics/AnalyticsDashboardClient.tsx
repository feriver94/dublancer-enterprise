...
export function AnalyticsDashboardClient({ canBackfill }: { canBackfill: boolean }) {
  ...
  return <main className="py-9 lg:py-14">
    <header className="flex flex-wrap items-end justify-between gap-4"><div className="min-w-0"><p className="font-bold uppercase tracking-[.18em] text-[#009A44]">{t("eyebrow")}</p><h1 className="text-3xl font-bold text-[#0F4C5C] sm:text-4xl xl:text-5xl">{t("title")}</h1><p className="mt-2 max-w-4xl text-slate-600">{t("description")}</p></div><span className={`max-w-full rounded-full px-4 py-2 text-xs font-bold ${summary?.freshness.stale ? "bg-amber-50 text-amber-700" : "bg-emerald-50 text-emerald-700"}`}>{summary?.freshness.stale ? t("stale") : t("fresh")} · {dateTime(summary?.freshness.latestCompletedAt)}</span></header>
    ...
    <section className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-5">{[
