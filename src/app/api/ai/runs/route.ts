...
export async function GET() { try { const context = await getAuthenticatedContext(); await requirePermission(context, "ai.use"); return apiSuccess(await service.list(context)); } catch (error) { return apiError(error); } }
