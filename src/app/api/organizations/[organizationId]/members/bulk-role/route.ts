...
import { bulkRoleChangeSchema } from "@/lib/validation/phase7";
...
export async function PATCH(request: NextRequest, route: RouteContext) {
  try {
    ...
    const input = bulkRoleChangeSchema.parse(await request.json());
    return apiSuccess(await service.bulkRoleChange(context, input.membershipIds, input.roleId));
  } catch (error) {
