...
import { governedReviewSchema } from "@/lib/validation/phase-c";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ contractId: string }> }) { try { await requireCsrfToken(request); return apiSuccess(await service.createReview(await getAuthenticatedContext(), (await params).contractId, governedReviewSchema.parse(await request.json())), 201); } catch (error) { return apiError(error); } }
