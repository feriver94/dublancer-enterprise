...
export async function POST(request: NextRequest, { params }: { params: Promise<{ jobId: string }> }) { try { await requireCsrfToken(request); const input = jobActionSchema.parse(await request.json()); return apiSuccess(await service.cancelJob(await getAuthenticatedContext(), (await params).jobId, input.reason)); } catch (error) { return apiError(error); } }
