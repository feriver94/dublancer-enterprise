import type { NextRequest } from "next/server";
import { apiError, apiSuccess } from "@/lib/http/api-response";
import { getAuthenticatedContext } from "@/lib/auth/session";
import { requireCsrfToken } from "@/lib/auth/csrf";
import { ChatMessageService } from "@/lib/services/chat-message.service";
import { updateChatMessageSchema } from "@/lib/validation/chat";

type RouteContext = { params: Promise<{ channelId: string; messageId: string }> };
const service = new ChatMessageService();

export async function PATCH(request: NextRequest, route: RouteContext) {
  try {
    await requireCsrfToken(request);
    const context = await getAuthenticatedContext();
    const { channelId, messageId } = await route.params;
    const input = updateChatMessageSchema.parse(await request.json());
    return apiSuccess(await service.update(context, channelId, messageId, input));
  } catch (error) {
    return apiError(error);
  }
}

export async function DELETE(request: NextRequest, route: RouteContext) {
  try {
    await requireCsrfToken(request);
    const context = await getAuthenticatedContext();
    const { channelId, messageId } = await route.params;
    return apiSuccess(await service.remove(context, channelId, messageId));
  } catch (error) {
    return apiError(error);
  }
}
